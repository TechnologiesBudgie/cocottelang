        ...
        match &obj {
            ...
            Value::Str(s) => {
                ...
                match method {
                    ...
                    "split"       => { let sep = args.first().map(|v| v.to_display()).unwrap_or_default(); let parts = s.split(&sep as &str).map(|p| Value::Str(p.to_string())).collect(); Ok(Value::List(Arc::new(Mutex::new(parts)))) }
