...
fn build_for_target(
    ...
    // Write .cargo/config.toml with the correct cross-linker for this target.
    // Without this, cargo uses the host linker and immediately fails on cross builds.
    if !triple.is_empty() {
        write_cargo_config(&tmp_dir, &triple)?;
    }

    // Ensure the rustup target is installed before trying to build.
    // This is a no-op if already present.
    if !triple.is_empty() {
        ensure_rustup_target(&triple, opts.verbose);
    }
    ...
    match rt_src {
        Some(rt_path) => {
            ...
            // Disable the gui feature when cross-compiling — egui/eframe require
            // a native display server and cannot cross-compile to arbitrary targets.
            let enable_gui = triple.is_empty();
            setup_runtime_crate(&tmp_dir, &rt_path, opts.verbose, enable_gui)?;
            step("Writing", "runner binary crate");
            setup_runner_crate(&tmp_dir, source, &opts.project_name, enable_gui)?;
        }
        None => {
            if opts.verbose {
...
fn setup_runtime_crate(tmp_dir: &Path, rt_src: &Path, verbose: bool, enable_gui: bool) -> Result<()> {
    ...
    let lib_rs = generate_lib_rs(enable_gui);
    fs::write(src_dst.join("lib.rs"), lib_rs)?;

    let gui_deps = if enable_gui {
        r#"eframe     = { version = "0.29", optional = true, features = ["wgpu"] }
egui       = { version = "0.29", optional = true }"#
    } else {
        ""
    };
    let gui_features = if enable_gui {
        r#"
[features]
default = ["gui"]
gui     = ["eframe", "egui"]"#
...
fn setup_runner_crate(tmp_dir: &Path, source: &str, project_name: &str, enable_gui: bool) -> Result<()> {
...
"#);
    ...
    let rt_dep = if enable_gui {
        format!(r#"cocotte_rt = {{ path = "../cocotte_rt", features = ["gui"] }}"#)
    } else {
        format!(r#"cocotte_rt = {{ path = "../cocotte_rt" }}"#)
    };
...
fn generate_lib_rs(enable_gui: bool) -> String {
    let charlotte_mod = if enable_gui {
        "#[cfg(feature = \"gui\")]\npub mod charlotte;"
    } else {
        "// charlotte (gui) disabled for cross-compilation target"
    };
    format!(r#"// Auto-generated lib.rs — exposes cocotte runtime modules
pub mod ast;
pub mod lexer;
pub mod parser;
pub mod error;
...
/// Write .cargo/config.toml into the temp workspace with the correct
/// cross-linker for the given target triple.
/// Without this, cargo uses the host linker and immediately fails.
fn write_cargo_config(tmp_dir: &Path, triple: &str) -> Result<()> {
    let config_dir = tmp_dir.join(".cargo");
    fs::create_dir_all(&config_dir)?;

    // Map triple → system cross-linker binary name.
    // These are the standard names from the GNU cross-toolchain packages
    // available on Debian/Ubuntu via apt-get.
    let linker = match triple {
        ...
        "riscv64gc-unknown-linux-gnu"    => Some("riscv64-linux-gnu-gcc"),
        "x86_64-pc-windows-gnu"          => Some("x86_64-w64-mingw32-gcc"),
        "i686-pc-windows-gnu"            => Some("i686-w64-mingw32-gcc"),
        // macOS and MSVC targets need special toolchains — no system linker
        _ => None,
    };

    let mut content = String::from("[build]\n");

    if let Some(lnk) = linker {
        // Only set the linker if the binary is actually present on this system.
        // If it's missing, cargo will give a clear error pointing to the right package.
        let lnk_present = std::process::Command::new("which")
            .arg(lnk)
        ...
        if lnk_present {
            ...
            // Emit the config entry anyway so cargo gives the right error,
            // but also print a helpful install hint.
            content.push_str(&format!(
                "\n[target.{}]\nlinker = \"{}\"\n",
                triple, lnk
            ));
            let pkg = linker_package_hint(triple);
            step_warn("Note", &format!(
                "cross-linker '{}' not found — install with:", lnk
            ));
            substep(&format!("sudo apt-get install {}", pkg));
        }
    } else if triple.contains("apple") {
        step_warn("Note", "macOS cross-compilation requires osxcross");
        substep("see: https://github.com/tpoechtrager/osxcross");
        substep("emitting source bundle instead — compile on a Mac");
        return Err(CocotteError::build_err(
            "macOS cross-compilation requires osxcross — emitting source bundle"
        ));
    } else if triple.contains("msvc") {
        step_warn("Note", "Windows MSVC cross-compilation not supported from Linux/macOS");
        substep("use --os windows --arch x86_64 which defaults to the GNU ABI (mingw)");
        return Err(CocotteError::build_err(
            "MSVC cross-compilation not supported — use GNU target instead"
        ));
    }
...
/// Return the apt package name for a cross-linker triple.
fn linker_package_hint(triple: &str) -> &'static str {
    match triple {
        "x86_64-unknown-linux-gnu"      => "gcc-x86-64-linux-gnu",
        "aarch64-unknown-linux-gnu"     => "gcc-aarch64-linux-gnu",
        "armv7-unknown-linux-gnueabihf" => "gcc-arm-linux-gnueabihf",
        "i686-unknown-linux-gnu"        => "gcc-i686-linux-gnu",
        "riscv64gc-unknown-linux-gnu"   => "gcc-riscv64-linux-gnu",
        "x86_64-pc-windows-gnu"         => "gcc-mingw-w64-x86-64",
        "i686-pc-windows-gnu"           => "gcc-mingw-w64-i686",
        _                               => "gcc (cross toolchain)",
    }
}

/// Ensure the rustup target is installed. Calls `rustup target add <triple>`.
/// Silently succeeds if rustup is not present (cargo might work without it).
fn ensure_rustup_target(triple: &str, verbose: bool) {
    if verbose {
        substep(&format!("ensuring rustup target: {}", triple));
    }
    // Only bother if rustup is on PATH
    let has_rustup = std::process::Command::new("rustup")
        .arg("--version")
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null())
        .status()
    ...
    if let Ok(s) = status {
        if s.success() && verbose {
            substep(&format!("target {} is ready", triple));
        }
    }
}
...
fn emit_source_bundle(opts: &BuildOptions, source: &str, target: &str) -> Result<()> {
    ...
    if let Some(rt_src) = locate_runtime_src() {
        ...
        fs::write(rt_dst.join("lib.rs"), generate_lib_rs(true))?;
