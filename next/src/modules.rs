...
fn make_http_module() -> Value {
    ...
    // Starts a synchronous HTTP server on the given port.  Blocks forever.
    //
    // Request map:  { "method", "path", "query", "headers", "body" }
    // Response map: { "status"?, "body"?, "headers"? }
    ns.insert("serve".into(), Value::NativeFunction(NativeFunction {
        ...
        func: Arc::new(|args| {
            ...
            crate::http_server::run_server(port, handler)
                .map(|_| Value::Nil)
        }),
    }));
    ...
    ns.insert("serve_static".into(), Value::NativeFunction(NativeFunction {
        ...
        func: Arc::new(|args| {
            let port = match args.first() {
                Some(Value::Number(n)) => *n as u16,
                _ => return Err(CocotteError::type_err("http.serve_static(port, dir) — port must be a number")),
            };
            let dir = match args.get(1) {
                Some(Value::Str(s)) => s.clone(),
                _ => return Err(CocotteError::type_err("http.serve_static(port, dir) — dir must be a string")),
            };
            crate::http_server::run_static_server(port, &dir)
                .map(|_| Value::Nil)
        }),
    }));
